...
class MySQLTaskRepository(_BaseMySQLRepository):
    ...
    def _select_tasks_base_sql() -> str:
        return """
            SELECT
                ...
                COALESCE(t.source_time_range, ''),
                NULL AS source_language_code,
                t.response_language_code,
                COALESCE(t.style_code, ''),
                COALESCE(t.content_length_code, ''),
                t.footer_text,
                t.footer_link_url,
    ...
    def _map_task(row: tuple[Any, ...]) -> Task:
        return Task(
            ...
            source_time_range=str(row[7] or ""),
            source_language_code=str(row[8]) if row[8] is not None else None,
            response_language_code=str(row[9]),
            style_code=str(row[10] or ""),
            content_length_code=str(row[11] or ""),
            footer_text=str(row[12]) if row[12] is not None else None,
            footer_link_url=str(row[13]) if row[13] is not None else None,
            scheduled_publish_at=row[14],
            publish_mode=str(row[15]),
            article_cost=int(row[16] or 1),
            billing_state=TaskBillingState(str(row[17])),
            task_status=TaskStatus(str(row[18])),
            retry_count=int(row[19] or 0),
            last_error_message=str(row[20]) if row[20] is not None else None,
            next_attempt_at=row[21],
            claimed_by=str(row[22]) if row[22] is not None else None,
            claimed_at=row[23],
            lease_until=row[24],
            completed_at=row[25],
        )
