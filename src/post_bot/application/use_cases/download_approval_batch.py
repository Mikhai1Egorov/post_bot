        ...
        try:
            with self._uow:
                ...
                if batch.batch_status == ApprovalBatchStatus.DOWNLOADED:
                    zip_artifact = self._uow.artifacts.get_by_id(batch.zip_artifact_id) if batch.zip_artifact_id is not None else None
                    task_ids = tuple(self._uow.approval_batch_items.list_task_ids(batch.id)[:1])
                    self._uow.commit()
                    return DownloadApprovalBatchResult(
                        batch_id=command.batch_id,
                        success=True,
                        task_ids=task_ids,
                        zip_storage_path=zip_artifact.storage_path if zip_artifact is not None else None,
                        zip_file_name=zip_artifact.file_name if zip_artifact is not None else None,
                        error_code=None,
                    )
            ...
            log_event(
                ...
                extra={"batch_id": command.batch_id, "tasks_count": 1, "user_id": command.user_id},
            )
            return DownloadApprovalBatchResult(
                batch_id=command.batch_id,
                success=True,
